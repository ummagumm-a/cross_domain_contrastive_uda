...
def train(source_dataset, target_dataset, num_classes, settings, version, total_epochs=500, remove_mismatched=False):
    ...
    if remove_mismatched:
        target_dataset = RemoveMismatchedAdapter(target_dataset)
    ...
    model = UDAModel(resnet, classification_head, num_classes, 
                     ...
                     class_names=source_dataset.get_class_names())

    tb_logger = TensorBoardLogger('lightning_logs', 'new_final', version=version)
...
#        ])
    ...
    # load transforms good for the pretrained model
    from torchvision.models import ResNet50_Weights
    ...
    dslr_dataset = OfficeDataset('dslr', transform=transform)
    ...
    for i in range(3, num_simulations + 1):
